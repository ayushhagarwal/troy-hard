...
await page.getByRole("button", { name: "BEGIN THE INFILTRATION" }).waitFor()
assert.equal(await page.getByRole("button", { name: "BEGIN THE INFILTRATION" }).isVisible(), true)
...
await page.getByRole("button", { name: "BEGIN THE INFILTRATION" }).click()
...
assert.ok(afterInput.progressM > beforeInput.progressM, "the Trojan pull team must advance automatically")
assert.ok(afterInput.braceLoad > beforeInput.braceLoad, "Space must brace the hidden crew")
...
await page.getByText("TORCH RAISED · THE TROJANS ARE STOPPING").waitFor()
...
await page.getByText("INSPECTION · KEEP THE HIDDEN CREW STILL").waitFor()
...
await phone.getByRole("button", { name: "BEGIN THE INFILTRATION" }).tap()
...
await compactPhone.getByRole("button", { name: "BEGIN THE INFILTRATION" }).tap()
